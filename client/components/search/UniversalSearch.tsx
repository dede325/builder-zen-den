...
interface SearchResult {
  id: string;
  type: 'patient' | 'appointment' | 'exam' | 'medical_record' | 'medication' | 'doctor' | 'message';
  title: string;
  subtitle: string;
  description: string;
...
export default function UniversalSearch({
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    const filtered = mockResults.filter(result => {
      const searchText = `${result.title} ${result.subtitle} ${result.description} ${result.tags?.join(' ') || ''}`.toLowerCase();
      ...
      const filterMatch = activeFilters.length === 0 || activeFilters.every(filter => {
        switch (filter.category) {
          ...
          case 'dateRange':
            ...
            const now = new Date();
            const days = Math.floor((now.getTime() - resultDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    const newSuggestions = searchSuggestions.filter(s => 
      s.toLowerCase().includes(searchQuery.toLowerCase()) && s !== searchQuery
    );
  ...
  const generateFilters = (searchResults: SearchResult[]) => {
    ...
    // Status filter
    const statuses = [...new Set(searchResults.map(r => r.status).filter(Boolean))];
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {availableFilters
                            ...
                            .map(filter => (
                              <div key={`${filter.category}-${filter.value}`} 
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex flex-col items-end space-y-1 ml-4">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {format(new Date(result.lastUpdated), 'dd/MM HH:mm')}
                          </span>
