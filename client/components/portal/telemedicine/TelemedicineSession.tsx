...
export default function TelemedicineSession({
  ...
  const createPeerConnection = (participantId: string): RTCPeerConnection => {
    ...
    pc.ontrack = (event) => {
      ...
      setRemoteStreams(
        (prev) => new Map(prev.set(participantId, remoteStream)),
      );
    ...
    pc.onconnectionstatechange = () => {
      console.log(
        `[Telemedicine] Connection state with ${participantId}:`,
        pc.connectionState,
      );
  ...
  const uploadRecording = async (recordingBlob: Blob) => {
    try {
      ...
      if (response.ok) {
        ...
        setSessionData((prev) =>
          prev ? { ...prev, recordingUrl: result.url } : null,
        );
  ...
  const checkNetworkQuality = () => {
    ...
    const connectedPeers = Array.from(peerConnections.current.values()).filter(
