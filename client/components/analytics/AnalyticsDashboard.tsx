...
interface AnalyticsData {
  ...
  trends: {
    ...
    departments: Array<{ name: string; appointments: number; revenue: number; color: string }>;
  };
  performance: {
    doctorStats: Array<{ name: string; appointments: number; rating: number; revenue: number }>;
    appointmentStats: Array<{ hour: string; scheduled: number; completed: number; cancelled: number }>;
  };
  demographics: {
...
export default function AnalyticsDashboard() {
  ...
  const [data, setData] = useState<AnalyticsData>({
    ...
    trends: {
      ...
      departments: [
        { name: 'Cardiologia', appointments: 245, revenue: 122500, color: '#ff6b6b' },
        { name: 'Pediatria', appointments: 198, revenue: 99000, color: '#4ecdc4' },
        { name: 'Dermatologia', appointments: 167, revenue: 83500, color: '#45b7d1' },
        { name: 'Clínica Geral', appointments: 156, revenue: 78000, color: '#96ceb4' },
        { name: 'Neurologia', appointments: 90, revenue: 45000, color: '#feca57' },
      ],
    },
    performance: {
      doctorStats: [
        { name: 'Dr. António Silva', appointments: 156, rating: 4.8, revenue: 78000 },
        { name: 'Dra. Maria Santos', appointments: 142, rating: 4.6, revenue: 71000 },
        { name: 'Dr. João Mendes', appointments: 134, rating: 4.7, revenue: 67000 },
        { name: 'Dra. Ana Costa', appointments: 128, rating: 4.5, revenue: 64000 },
        { name: 'Dr. Pedro Lima', appointments: 98, rating: 4.4, revenue: 49000 },
      ],
      appointmentStats: [
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.overview.totalAppointments.toLocaleString()}</div>
            <div className="flex items-center text-xs text-green-600">
              <TrendingUp className="w-3 h-3 mr-1" />
              +8.2% vs mês anterior
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.overview.averageWaitTime}min</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.overview.patientSatisfaction}/5</div>
            <div className="flex items-center text-xs text-green-600">
              <TrendingUp className="w-3 h-3 mr-1" />
              +2.1% vs mês anterior
            </div>
                  ...
                  <div key={doctor.name} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{doctor.appointments} consultas</p>
                      </div>
                    </div>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={data.trends.patients}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Line type="monotone" dataKey="new" stroke="#8884d8" name="Novos" />
                    <Line type="monotone" dataKey="returning" stroke="#82ca9d" name="Retornos" />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
