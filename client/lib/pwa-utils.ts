...
class PWAManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains("appointment-forms")) {
          const store = db.createObjectStore("appointment-forms", {
            keyPath: "id",
          });
          store.createIndex("timestamp", "timestamp");
        }
  ...
  public isBusinessHours(date: Date = new Date()): boolean {
    ...
    if (dayOfWeek === 6) {
      // Saturday
      return (
        time >= ANGOLA_CONFIG.businessHours.saturday.start &&
    ...
    // Weekdays
    return (
      time >= ANGOLA_CONFIG.businessHours.weekdays.start &&
