...
export const checkPermissions: RequestHandler = async (req, res) => {
  try {
    ...
    if (permission) {
      ...
      hasPermission = PermissionManager.canAccessResource(
        user,
        resource as string,
        action as string,
      );
...
export const getRolePermissions: RequestHandler = async (req, res) => {
  try {
    ...
    const permissions = PermissionManager.getPermissionsForRole(
      role as UserRole,
    );
...
export const validateAction: RequestHandler = async (req, res) => {
  try {
    ...
    let canPerformAction = PermissionManager.canAccessResource(
      user,
      resource,
      action,
    );
