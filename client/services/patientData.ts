...
export const mockExames: ExamePaciente[] = [
  {
    ...
    resultado_texto: 'Resultados dentro dos parâmetros normais. Contagem de glóbulos vermelhos: 4.5 milhões/μL, Hemoglobina: 14.2 g/dL.',
  ...
  },
  {
    ...
    resultado_texto: 'Ritmo sinusal regular. Frequência cardíaca: 72 bpm. Não foram detectadas alterações significativas.',
...
export const mockMensagens: Mensagem[] = [
  {
    ...
    texto: 'Olá Maria! Recebi os resultados dos seus exames. Está tudo dentro da normalidade.',
  ...
  },
  {
    ...
    texto: 'Que ótima notícia! Obrigada doutor. Quando devo agendar o próximo check-up?',
...
class PatientDataService {
  ...
  async updatePaciente(id: string, data: Partial<Paciente>): Promise<Paciente> {
    return new Promise((resolve) => {
      setTimeout(() => {
        const updated = { ...mockPaciente, ...data, updated_at: new Date().toISOString() };
        resolve(updated);
      }, 500);
    });
  }
  ...
  // Exames
  async getExames(pacienteId: string, filtros?: FiltroExames): Promise<ExamePaciente[]> {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        if (filtros?.periodo) {
          ...
          const dias = filtros.periodo === '7dias' ? 7 : filtros.periodo === '30dias' ? 30 : 365;
          const dataLimite = new Date(today.getTime() - (dias * 24 * 60 * 60 * 1000));
        ...
        if (filtros?.tipo) {
          exames = exames.filter(e => e.tipo.toLowerCase().includes(filtros.tipo!.toLowerCase()));
        }
        
        resolve(exames.sort((a, b) => new Date(b.data).getTime() - new Date(a.data).getTime()));
      }, 400);
    });
  }
  ...
  async getMensagens(conversaId: string): Promise<Mensagem[]> {
    return new Promise((resolve) => {
      setTimeout(() => {
        const mensagens = mockMensagens.filter(m => m.conversa_id === conversaId);
        resolve(mensagens.sort((a, b) => new Date(a.enviado_em).getTime() - new Date(b.enviado_em).getTime()));
      }, 400);
    });
  }
  ...
  async getFaturas(pacienteId: string): Promise<FaturaPaciente[]> {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        resolve(faturas.sort((a, b) => new Date(b.data_emissao).getTime() - new Date(a.data_emissao).getTime()));
      }, 400);
    });
  }
  ...
  // Upload de arquivo
  async uploadArquivo(file: File, tipo: 'foto' | 'documento'): Promise<{ success: boolean; url?: string; error?: string }> {
    return new Promise((resolve) => {
      setTimeout(() => {
        // Simular upload
        if (file.size > 5 * 1024 * 1024) { // 5MB
          resolve({ success: false, error: 'Arquivo muito grande. Máximo 5MB.' });
          return;
        }
