...
Equipe Bem Cuidar`,
  ...
  },
  {
    ...
    category: "appointment",
    content:
      "Lembrete: Consulta amanhã {{appointmentTime}} com {{doctorName}}. Chegue 15min antes. Bem Cuidar",
...
Equipe Bem Cuidar`,
  ...
  },
  {
    ...
    category: "emergency",
    content:
      "URGENTE: {{message}} - Entre em contato imediatamente: {{emergencyPhone}}",
...
const notificationLogs: NotificationLog[] = [
  ...
  },
  {
    ...
    content:
      "Lembrete: Consulta amanhã 14:30 com Dr. António Silva. Chegue 15min antes. Bem Cuidar",
    status: "sent",
  ...
  try {
    ...
    if (active !== undefined) {
      filtered = filtered.filter(
        (template) => template.active === (active === "true"),
      );
    }
...
export const getNotificationStats: RequestHandler = (req, res) => {
  try {
    ...
    if (startDate) {
      filtered = filtered.filter(
        (log) => new Date(log.createdAt) >= new Date(startDate as string),
      );
    }

    if (endDate) {
      filtered = filtered.filter(
        (log) => new Date(log.createdAt) <= new Date(endDate as string),
      );
    }
    ...
    const stats = {
      ...
      deliveryRate:
        filtered.length > 0
          ? (filtered.filter((log) => log.status === "delivered").length /
              filtered.length) *
            100
          : 0,
      readRate:
        filtered.length > 0
          ? (filtered.filter((log) => log.status === "read").length /
              filtered.length) *
            100
          : 0,
    };
