...
export const createAppointment: RequestHandler = async (req, res) => {
  try {
    ...
    const appointment = portalStorage.createAppointment(
      patient.id,
      appointmentData,
    );
    ...
    res.json({
      ...
      data: appointment,
      message:
        "Solicitação de consulta enviada com sucesso. Entraremos em contato para confirmação.",
    });
  } catch (error) {
...
export const updateAppointment: RequestHandler = async (req, res) => {
  try {
    ...
    const updatedAppointment = portalStorage.updateAppointment(
      id,
...
export const cancelAppointment: RequestHandler = async (req, res) => {
  try {
    ...
    const updatedAppointment = portalStorage.updateAppointment(id, {
      status: "cancelled",
    });
...
function getDoctorBySpecialty(specialty: string): string {
  ...
  return (
    doctors[specialty as keyof typeof doctors] || "Dr. Médico Especialista"
  );
}
