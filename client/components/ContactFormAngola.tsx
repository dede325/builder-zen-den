...
import { angolaFormatter, ANGOLA_PROVINCES, MAJOR_MUNICIPALITIES, FormLabels } from "@/lib/locale-angola";
...
export default function ContactFormAngola() {
  ...
  useEffect(() => {
    ...
    if (formData.municipality && formData.province) {
      const isValidMunicipality = MAJOR_MUNICIPALITIES[formData.province]?.includes(formData.municipality);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.phone.trim()) {
      ...
      errors.phone = 'Formato de telefone inválido para Angola (+244 XXX XXX XXX)';
    }
    ...
    if (!formData.dataConsent) {
      errors.dataConsent = 'É obrigatório consentir o tratamento de dados pessoais';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isOnline) {
        ...
        // Offline storage
        const formId = await pwaManager.storeFormOffline('contact', submissionData);
        
        setSubmitStatus({
          type: 'offline',
          message: 'Sem conexão à internet. Os seus dados foram guardados e serão enviados quando a conexão for restaurada.'
        });
      ...
      if (!isOnline) {
        try {
          ...
          setSubmitStatus({
            type: 'offline',
            message: 'Erro de conexão. Os seus dados foram guardados e serão enviados quando a conexão for restaurada.'
          });
        } catch (offlineError) {
          console.error('Failed to store form offline:', offlineError);
          setSubmitStatus({
            type: 'error',
            message: 'Erro ao guardar dados. Verifique sua conexão e tente novamente.'
          });
        }
      } else {
        setSubmitStatus({
  ...
  const isAppointmentRequest = formData.subject === 'consulta' || formData.subject === 'exame';
      ...
      <CardContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                >
                  <SelectTrigger className={`min-h-[44px] ${formErrors.province ? 'border-red-500' : ''}`}>
              ...
              <div>
                ...
                <Select 
                  ...
                  disabled={!formData.province || availableMunicipalities.length === 0}
                >
                  <SelectTrigger className={`min-h-[44px] ${formErrors.municipality ? 'border-red-500' : ''}`}>
                ...
                {formErrors.municipality && (
                  <p className="text-red-500 text-sm mt-1">{formErrors.municipality}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {isAppointmentRequest && (
              <>
                <div>
                  ...
                  >
                    <SelectTrigger className={`min-h-[44px] ${formErrors.specialty ? 'border-red-500' : ''}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      disabled={isSubmitting}
                      min={angolaFormatter.formatDate(new Date(), { day: '2-digit', month: '2-digit', year: 'numeric' }).split('/').reverse().join('-')}
                    />
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger className={`min-h-[44px] ${formErrors.preferredTime ? 'border-red-500' : ''}`}>
            ...
            <div>
              ...
              <Textarea
                ...
                disabled={isSubmitting}
                placeholder={isAppointmentRequest 
            ...
            <div className="space-y-3">
              <div className="flex items-start space-x-3">
                ...
                <div className="grid gap-1.5 leading-none">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Consinto o tratamento dos meus dados pessoais para fins de atendimento médico, conforme a Lei n.º 22/11 de Proteção de Dados Pessoais de Angola.
                  </p>
                  {formErrors.dataConsent && (
                    <p className="text-red-500 text-xs">{formErrors.dataConsent}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="grid gap-1.5 leading-none">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Desejo receber informações sobre novos serviços, promoções e conteúdo de saúde por e-mail ou SMS.
                  </p>
                </div>
              </div>
            </div>
          ...
          <p className="text-xs text-muted-foreground text-center">
            * Campos obrigatórios | Dados protegidos pela Lei n.º 22/11 de Angola
          </p>
        </form>
      </CardContent>
    </Card>
