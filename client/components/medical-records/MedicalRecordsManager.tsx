...
export default function MedicalRecordsManager() {
  ...
  const [medicalRecords] = useState<MedicalRecord[]>([
    {
      ...
      historyOfPresentIllness: 'Paciente relata dor torácica iniciada há 3 dias, tipo aperto, que piora com esforço físico. Associada a dispneia aos pequenos esforços.',
      physicalExamination: 'PA: 150/95 mmHg, FC: 85 bpm, FR: 18 irpm. Ausculta cardíaca: bulhas hipofonéticas, sem sopros. Ausculta pulmonar: murmúrio vesicular presente bilateralmente.',
      assessment: 'Síndrome coronariana aguda a esclarecer. Hipertensão arterial descompensada.',
      ...
      notes: 'Paciente colaborativo, orientado quanto aos fatores de risco cardiovascular.',
  ...
  const getBloodPressureStatus = (systolic: number, diastolic: number) => {
    ...
    if (systolic >= 130 || diastolic >= 80) {
      return { label: 'Pré-hipertensão', color: 'text-yellow-600', icon: TrendingUp };
    }
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">
                    {patient.gender === 'male' ? 'M' : 'F'}, {calculateAge(patient.birthDate)} anos
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  ...
                  <div className="flex items-center text-sm text-muted-foreground">
                    ...
                    {getPatientMedicalRecords(patient.id).length} registro(s) médico(s)
                  </div>
                  ...
                  {patient.allergies.length > 0 && (
                    <div className="flex items-start text-sm">
                      ...
                      <div>
                        <span className="text-red-600 font-medium">Alergias:</span>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {selectedPatient.gender === 'male' ? 'Masculino' : 'Feminino'}, {calculateAge(selectedPatient.birthDate)} anos • {selectedPatient.bloodType}
            </p>
          </div>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-xs ${getBMIStatus(selectedPatient.weight / (selectedPatient.height / 100) ** 2).color}`}>
                  {getBMIStatus(selectedPatient.weight / (selectedPatient.height / 100) ** 2).label}
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl font-bold">{selectedPatient.allergies.length}</p>
                <p className="text-xs text-muted-foreground">
                  {selectedPatient.allergies.length > 0 ? selectedPatient.allergies.join(', ') : 'Nenhuma'}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-xl font-bold">
                  {getPatientMedicalRecords(selectedPatient.id).length > 0
                    ? format(new Date(getPatientMedicalRecords(selectedPatient.id)[0].date), 'dd/MM')
                    : 'N/A'
                  }
                </p>
                <p className="text-xs text-muted-foreground">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-6">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{selectedPatient.height} cm</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium">Peso</Label>
                    <p className="text-sm text-muted-foreground">{selectedPatient.weight} kg</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Válido até: {format(new Date(selectedPatient.insurance.validUntil), 'dd/MM/yyyy')}
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                {getPatientVitalSigns(selectedPatient.id).length > 0 ? (
                  <div className="space-y-4">
                    {(() => {
                      const latest = getPatientVitalSigns(selectedPatient.id)[0];
                      const bpStatus = getBloodPressureStatus(latest.bloodPressureSystolic, latest.bloodPressureDiastolic);
                      const StatusIcon = bpStatus.icon;
                      
                      return (
                        <>
                          <div className="text-sm text-muted-foreground">
                            {format(new Date(latest.date), 'dd/MM/yyyy HH:mm', { locale: ptBR })}
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center space-x-1">
                                  <StatusIcon className={`w-4 h-4 ${bpStatus.color}`} />
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <span className="font-medium">{latest.heartRate} bpm</span>
                              </div>
                              
                              <div className="flex items-center justify-between">
                                <span className="text-sm">Temperatura</span>
                                <span className="font-medium">{latest.temperature}°C</span>
                              </div>
                            </div>
                            ...
                            <div className="space-y-2">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="font-medium">{latest.oxygenSaturation}%</span>
                              </div>
                              
                              <div className="flex items-center justify-between">
                                <span className="text-sm">Freq. Respiratória</span>
                                <span className="font-medium">{latest.respiratoryRate} irpm</span>
                              </div>
                              
                              {latest.pain !== undefined && (
                                <div className="flex items-center justify-between">
                                  <span className="text-sm">Dor (0-10)</span>
                                  <span className="font-medium">{latest.pain}</span>
                                </div>
                              )}
                            </div>
                          </div>
          ...
          <Card>
            ...
            <CardContent>
              {getPatientMedicalRecords(selectedPatient.id).slice(0, 3).map(record => (
                <div key={record.id} className="border-l-4 border-blue-500 pl-4 py-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-1">
                        ...
                        <span className="text-sm text-muted-foreground">• {record.doctorName}</span>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Badge className={
                  ...
                  <Accordion type="single" collapsible>
                    <AccordionItem value="details">
                      ...
                      <AccordionContent className="space-y-4">
                        <div>
                          <Label className="font-medium">História da Doença Atual</Label>
                          <p className="text-sm mt-1">{record.historyOfPresentIllness}</p>
                        </div>
                        
                        <div>
                          <Label className="font-medium">Exame Físico</Label>
                          <p className="text-sm mt-1">{record.physicalExamination}</p>
                        </div>
                        ...
                        <div>
                          <Label className="font-medium">Plano de Tratamento</Label>
