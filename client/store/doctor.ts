...
interface DoctorState {
  ...
  createConsultation: (
    consultation: Omit<DoctorConsultation, "id" | "createdAt" | "updatedAt">,
  ) => Promise<string>;
  updateConsultation: (
    consultationId: string,
    updates: Partial<DoctorConsultation>,
  ) => Promise<void>;
  ...
  requestExam: (
    exam: Omit<MedicalExam, "id" | "createdAt" | "updatedAt">,
  ) => Promise<string>;
  updateExamResults: (
    examId: string,
    results: string,
  ...
  sendMessage: (
    message: Omit<InternalMessage, "id" | "createdAt" | "updatedAt">,
  ) => Promise<string>;
...
const mockMessages: InternalMessage[] = [
  {
    ...
    content:
      "Dr. António, a paciente Maria Santos chegou com dor torácica intensa. Está na sala 3 aguardando avaliação urgente.",
    priority: "urgent",
    type: "urgent_care",
    read: false,
    replied: false,
  ...
  },
  {
    ...
    content:
      "Dr. António, o paciente José Mendes solicitou reagendamento da consulta das 10:30 de amanhã para 15:00. Podemos confirmar?",
    priority: "normal",
  ...
  },
  {
    ...
    content:
      "Colega, paciente Ana Paula apresentou alterações no pós-operatório que gostaria de discutir. Podemos conversar hoje?",
    priority: "high",
    type: "patient_update",
    read: false,
    replied: false,
...
export const useDoctorStore = create<DoctorState>((set, get) => ({
  ...
  fetchConsultations: async (doctorId: string) => {
    ...
    const doctorConsultations = mockConsultations.filter(
  ...
  deleteConsultation: async (consultationId: string) => {
    const currentConsultations = get().consultations;
    const filteredConsultations = currentConsultations.filter(
  ...
  fetchMessages: async (doctorId: string) => {
    ...
    const doctorMessages = mockMessages.filter(
      (message) =>
