...
import { DashboardStats, Consulta, Exame, ItemEstoque } from '@shared/manager-types';
...
function QuickActionCard({ title, description, icon: Icon, action, color = 'bg-primary' }: QuickActionCardProps) {
  return (
...
export function ManagerDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [statsData, consultasData, examesData, estoqueData] = await Promise.all([
        ...
        setStats(statsData);
        setConsultasHoje(consultasData.filter(c => c.data === new Date().toISOString().split('T')[0]));
        setExamesPendentes(examesData.filter(e => e.status === 'pendente'));
        setEstoqueAlerta(estoqueData.filter(i => i.quantidade <= i.quantidadeMinima));
      } catch (error) {
            ...
            <div className="space-y-3">
              {consultasHoje.slice(0, 3).map((consulta) => (
                <div key={consulta.id} className="flex items-center justify-between border-b pb-2">
                  <div>
                    <p className="font-medium text-sm">{consulta.pacienteNome}</p>
                    <p className="text-xs text-muted-foreground">
                      {consulta.hora} - {consulta.especialidade}
                    </p>
                  </div>
                  <Badge variant={consulta.status === 'em_andamento' ? 'default' : 'secondary'}>
          ...
          <CardContent>
            <div className="space-y-3">
              {examesPendentes.slice(0, 3).map((exame) => (
                <div key={exame.id} className="flex items-center justify-between border-b pb-2">
                  <div>
